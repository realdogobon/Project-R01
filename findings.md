...
The reported Vite console message came from a failed initial secure WebSocket upgrade through the managed preview, followed by Vite’s internal fallback toward `localhost:5173`, which is unreachable from a user browser. The Express/Vite setup intentionally shares one HTTP server with the managed proxy. The correction preserves that shared server and explicitly configures the browser HMR client to use the proxy’s secure endpoint: `wss` on port `443`. No fixed public hostname is used, so the currently managed preview domain remains the browser-visible origin.

After restarting the development service, a fresh browser session reached the managed preview with `[vite] connecting…` followed by `[vite] connected.` and no direct-WebSocket fallback, page error, or browser-console HMR error. The new `scripts/vite_hmr_connection_probe.mjs` checks this condition in future. TypeScript, all 9 Vitest assertions, the production build, and whitespace validation passed after the configuration correction.

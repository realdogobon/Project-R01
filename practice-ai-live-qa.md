...
## Short-response recovery and silent exhaustion verification

On 2026-08-14 UTC, a delayed no-credit provider mock reproduced the reported scenario: a 400-word Medium request received repeated 111-word responses. The enforcement flow made five bounded requests in total—one initial response, one corrective replacement, and three continuations—then produced a 432-word accepted result. No raw exception, browser error, or user-visible message was emitted on desktop or mobile.

An additional exhausted-recovery case requested 500 words while every mocked response remained 111 words. After the same five bounded requests, the prior Configure Session text remained unchanged, the modal returned to an idle Generate state, and the short-response text was absent from the document and browser error capture. Explicit Cancel, close/reopen persistence, smooth spinner behavior, the 14-subject/121-topic heavyweight matrix, Settings/modal regressions, TypeScript, and production build remained passing. No live provider calls were used in this verification.

## Gemini 3.7 Flash default-model validation

On 2026-08-14 UTC, the active Gemini account was checked without reading or returning its key. It listed both `gemini-3.7-flash` and `gemini-3.6-flash` as available for the current `generateContent` path. A controlled 400-word Practice-like request returned HTTP 200 and keyboard-safe text from both models, but each single response was intentionally below RoyScript’s accepted length window. The existing corrective-and-continuation enforcement therefore remains required. The single sample measured 7.9 seconds for Gemini 3.7 Flash and 11.4 seconds for Gemini 3.6 Flash; it is evidence of compatibility, not a broad performance guarantee.

The scanner-style image compatibility check used the exact Gemini image payload on a browser-generated text-and-table fixture. Both candidates returned HTTP 200, keyboard-safe OCR, and all expected unique fixture tokens. Gemini 3.7 Flash completed in 3.23 seconds and Gemini 3.6 Flash in 4.48 seconds. A separate minimal text request yielded a normal text part from Gemini 3.7 Flash while Gemini 3.6 Flash did not return a normal text part under the same contract. Gemini 3.7 Flash is therefore now the default for both Practice Text and scanner OCR. The existing scanner selector was updated only to include the truthful selected model; no layout or interaction changed. No key, private document, or raw model output is present in this report or the repository.

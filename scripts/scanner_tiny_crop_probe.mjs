...
const outputPath = process.argv[4] || "/tmp/scanner_tiny_crop_probe.json";
...
const readLayout = async (label) => page.evaluate((snapshotLabel) => {
  ...
  const deck = stage?.querySelector(":scope > div > div.relative.flex.flex-col.items-center.justify-center");
...
try {
  ...
  const before = await readLayout("before-tiny-crop");
  const surface = await page.$("[data-scanner-document-surface]");
  const surfaceBox = await surface.boundingBox();
  const start = { x: surfaceBox.x + surfaceBox.width * 0.22, y: surfaceBox.y + surfaceBox.height * 0.485 };
  const end = { x: surfaceBox.x + surfaceBox.width * 0.78, y: surfaceBox.y + surfaceBox.height * 0.515 };
  ...
  await sleep(160);
  const selected = await readLayout("after-tiny-crop");
  ...
  await sleep(220);
  const scanned = await readLayout("during-tiny-crop-scan");
